...
func TestApplicationsResponse_Next(t *testing.T) {
	page := int32(1)
	...
	s := startTestServer(t, func(w http.ResponseWriter, r *http.Request) {
		...
		if p == 3 {
			writeJSON(t, w, http.StatusOK, ApplicationsResponse{Data: []Application{{Id: int(p)}}})
			return
		}
		...
		writeJSON(t, w, http.StatusOK, ApplicationsResponse{
			paginatedResponse: paginatedResponse{
				Links: ResponseLinks{
					Next: r.URL.String(),
				},
			},
			Data: []Application{{Id: int(p)}},
		})
	})
	defer s.Close()
	...
	resp := ApplicationsResponse{
		paginatedResponse: paginatedResponse{
			Links: ResponseLinks{
				Next: fmt.Sprintf("%s/applications?page=1", s.URL),
			},
			service: &service{c},
		},
	}
	...
	for resp.Next(context.Background()) {
		require.Equal(t, []Application{{Id: i}}, resp.Data)
		i++
	}
