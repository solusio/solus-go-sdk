...
func TestLocationsResponse_Next(t *testing.T) {
	t.Run("positive", func(t *testing.T) {
		...
		s := startTestServer(t, func(w http.ResponseWriter, r *http.Request) {
			...
			if p == 3 {
				writeJSON(t, w, http.StatusOK, LocationsResponse{
					Data: []Location{
						{
							ID: int(p),
						},
		...
		resp := LocationsResponse{
			paginatedResponse: paginatedResponse{
				Links: ResponseLinks{
					Next: fmt.Sprintf("%s/locations?page=1", s.URL),
				},
				Meta: ResponseMeta{
					CurrentPage: 1,
					LastPage:    3,
		...
		for resp.Next(context.Background()) {
			require.Equal(t, []Location{{ID: i}}, resp.Data)
			i++
		}
	...
	t.Run("negative", func(t *testing.T) {
		t.Run("failed to make request", func(t *testing.T) {
			...
			resp := LocationsResponse{
				paginatedResponse: paginatedResponse{
					Links: ResponseLinks{
						Next: fmt.Sprintf("%s/locations?page=1", addr),
					},
					Meta: ResponseMeta{
						CurrentPage: 1,
						LastPage:    3,
			...
			asserter(t, http.MethodGet, "/locations?page=1", resp.Err())
		})
		...
		t.Run("invalid status code", func(t *testing.T) {
			...
			assert.EqualError(t, resp.Err(), fmt.Sprintf(
				"HTTP GET %s/locations?page=1 returns 400 status code",
				s.URL,
			))
		})
