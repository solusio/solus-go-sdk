...
func TestPermissionResponse_Next(t *testing.T) {
	t.Run("positive", func(t *testing.T) {
		...
		s := startTestServer(t, func(w http.ResponseWriter, r *http.Request) {
			...
			if p == 3 {
				writeJSON(t, w, http.StatusOK, PermissionResponse{
					Data: []Permission{
						{
							ID: int(p),
						},
		...
		resp := PermissionResponse{
			paginatedResponse: paginatedResponse{
				Links: ResponseLinks{
					Next: fmt.Sprintf("%s/permission?page=1", s.URL),
				},
				Meta: ResponseMeta{
					CurrentPage: 1,
					LastPage:    3,
		...
		for resp.Next(context.Background()) {
			require.Equal(t, []Permission{{ID: i}}, resp.Data)
			i++
		}
	...
	t.Run("negative", func(t *testing.T) {
		t.Run("failed to make request", func(t *testing.T) {
			...
			resp := PermissionResponse{
				paginatedResponse: paginatedResponse{
					Links: ResponseLinks{
						Next: fmt.Sprintf("%s/permission?page=1", addr),
					},
					Meta: ResponseMeta{
						CurrentPage: 1,
						LastPage:    3,
			...
			asserter(t, http.MethodGet, "/permission?page=1", resp.Err())
		})
